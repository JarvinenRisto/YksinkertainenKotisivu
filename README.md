...
POST pyyntöraja, 10 pyyntöä per ip / per minuutti ja jossa huonoa voi olla jos jaetussa verkossa samaan aikaan lähetetään lomake että raja tulee vastaan (fail2ban parempi). Onko sama viesti md5 tarkistus (md5 vanhentunut, SHA256 suositeltu), jos on merkitään meneväksi spam laatikkoon admin sivulle, josta voi vastata/lukea/poistaa. Jos käyttäjä lähetti lomakkeen nopeammin kuin 3 sekunnissa, lähetys estyy, $SESSION avulla toteutettu. Onko POST pyyntö laitettu sivujen lomakkeen kautta tarkistus, $SESSION tässäkin. $SESSION kohtainen 20 sekunnin viive edellisestä käyttäjän lomakkeen lähetyksestä. $SESSION voi olla huonoa että käyttäjä tarvitsee selaimen keksit (cookies) päällä. CSRF tarkistus. Käyttäjän lomakeviestin minimipituus 8 merkkiä, max pituus 4000. 
